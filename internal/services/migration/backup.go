...
func MigrateBackup(tntsrv interfaces.TenantManager, stgf interfaces.StorageFactory) error {
	err := tntsrv.GetTenants(func(t string) bool {
		log.Root.Debugf("BckMgr: found tenant: %s", t)
		...
		if ok {
			if mainstg.BckSrv == nil {
				log.Root.Debugf("no backstorage found for tenant %s", t)
				return true
			}
...
func migrateBckTnt(stg interfaces.BlobStorage, bck interfaces.BlobStorage) error {
	log.Root.Infof("starting backup migration for tenant: %s", stg.GetTenant())
	stg.GetBlobs(func(id string) bool {
		found, err := bck.HasBlob(id)
		if err != nil {
			log.Root.Errorf("error checking blob from backup storage %s: %s\r\n%v ", bck.GetTenant(), id, err)
		}
		if !found {
			log.Root.Infof("migrating file for tenant: %s, %s", stg.GetTenant(), id)
			wg.Add(1)
			go func() {
				defer wg.Done()
				backup(id, stg, bck)
